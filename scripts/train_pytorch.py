...
def load_checkpoint(model, optimizer, checkpoint_dir, device, ema_model=None):
    ...
    checkpoint_steps = [d for d in checkpoint_dir.iterdir() if d.is_dir() and d.name.isdigit()]
    ...
    try:
        ...
        (model.module if isinstance(model, torch.nn.parallel.DistributedDataParallel) else model).load_state_dict(
            model_state_dict
        )
...
def train_loop(config: _config.TrainConfig):
    ...
    if use_ddp:
        ...
        model = torch.nn.parallel.DistributedDataParallel(
            model, device_ids=[device.index] if device.type == "cuda" else None, find_unused_parameters=True
        )
    ...
    if config.pytorch_weight_path is not None:
        ...
        model_path = os.path.join(config.pytorch_weight_path, "model.safetensors")  # noqa: PTH118
            ...
            if ema_model is not None:
                try:
                    with torch.no_grad():
                        ...
                        for param, ema_param in zip(main_model_params, ema_model.parameters(), strict=True):
