...
"""
Timing sweep script for Attention inference across different sequence lengths.
Compares default (FSDP) vs megatron (tensor parallel) sharding strategies.

Usage: python attention_timing_sweep.py --output-dir /path/to/results
"""
...
def run_timing_test(seq_len, sharding_strategy, batch_size=2, num_shards=2, model_dim=4096, head_dim=128, dryrun=False):
    ...
    # Build command
    cmd = [
        "uv", "run", "scripts/kmalik2/test_attention_inference.py",
        "--mode", "timing",
        "--seq-len", str(seq_len),
...
def save_detailed_output(result, output_dir):
    ...
    with open(summary_file, 'w') as f:
        ...
        f.write(f"  Head Dim: {result['head_dim']}\n")
        f.write(f"  Num Heads: {result['num_heads']}\n")
        f.write(f"\nTiming Results:\n")
...
def append_to_csv(result, csv_file, write_header=False):
    ...
    fieldnames = [
        ...
        'model_dim', 'head_dim', 'num_heads', 'mean_time_ms', 'std_time_ms', 
...
def main():
    parser = argparse.ArgumentParser(description="Timing sweep for Attention sharding strategies")
    ...
    csv_file = output_dir / "attention_timing_results.csv"
    if csv_file.exists():
        csv_file.unlink()
    ...
    # Calculate num_heads for display
    num_heads = args.model_dim // args.head_dim
    
    print("=" * 60)
    print("ATTENTION TIMING SWEEP")
    print("=" * 60)
    ...
    # Final summary
    print("=" * 60)
    print(f"ATTENTION TIMING SWEEP {'DRY RUN ' if args.dryrun else ''}COMPLETED")
    print("=" * 60)
