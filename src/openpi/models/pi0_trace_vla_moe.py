...
class Pi0TraceVLAMoe(_model.BaseModel):
    """Trace-augmented VLA with MoE on both the action and the trace heads."""

    def __init__(self, config: _config.Pi0TraceVLAMoeConfig, rngs: nnx.Rngs):
        super().__init__(config.action_dim, config.action_horizon, config.max_token_len)
        self.config = config
        self.pi05 = config.pi05
        if not self.pi05:
            raise ValueError("Pi0TraceVLAMoe assumes pi05=True (adaRMS pathway).")
        ...
        # Optional appended semantic-target anchor row.
        ...
        # Skill semantics: both MoEs are routed by the same skill-id one-hot, so K
        # must agree on both streams.
        if self.num_action_experts != self.num_trace_experts:
            raise ValueError(
                f"num_action_experts ({self.num_action_experts}) must equal "
                f"num_trace_experts ({self.num_trace_experts}); both MoE streams are "
                f"routed by the same skill-id one-hot."
            )
        # Shared K used wherever the routing one-hot is built / completion head sized.
        self.K = self.num_action_experts
        ...
        # TraceBlock auto-dispatches per-stream FFN by config.num_local_experts: both
        # stream 1 and stream 2 get HardMoeBlock; stream 0 stays dense.
        ...
        # adaRMS on streams 1 (action) and 2 (trace) — matches TraceVLA / actionmoe.
    ...
    def _completion_predict(
        ...
        skill_one_hot = jax.nn.one_hot(skill_id, self.K, dtype=out.dtype)
    ...
    def _combine_weights(self, batch_size: int, skill_id: at.Int[at.Array, " b"], length: int) -> at.Float[at.Array, "b _t _k"]:
        ...
        """
        skill_one_hot = jax.nn.one_hot(skill_id, self.K)
        return jnp.broadcast_to(
            skill_one_hot[:, None, :], (batch_size, length, self.K)
        )
    ...
    def _dummy_combine_weights(self, batch_size: int) -> at.Float[at.Array, "b _t _k"]:
        ...
        """
        return jnp.zeros((batch_size, 1, self.K), dtype=jnp.dtype(self.config.dtype))
